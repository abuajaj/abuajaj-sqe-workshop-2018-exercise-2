 ...
 */
class Node {
    constructor(line, type, title, condition = '', value = '', kind = '', variableKind = '') {
        this.line         = line;
        this.type         = type;
        this.title        = title;
...
export default class Substitution {
    ...
    functionDeclarationNode(data) {
        let node = new Node(this.currentLine, data.type, 'function ' + data.id.name + '(');
        ...
        for (let i = 0; i < data.params.length; i++) {
            ...
            node.add(new Node(this.currentLine, 'VariableDeclaration', name, '', value, 'argument'));
    ...
    variableDeclarationNode(data, parentNode) {
        ...
        for (let i = 0; i < data.declarations.length; i++) {
            ...
            if (parentNode) parentNode.add(new Node(this.currentLine, 'VariableDeclaration', data.declarations[i].id.name, '', value, 'FunctionDeclaration' === parentNode.type ? 'local' : '', data.kind));
            else {
                parentNode = new Node(this.currentLine, 'VariableDeclaration', data.declarations[i].id.name, '', value, 'global', data.kind);
    ...
    expressionStatementNode(data, parentNode) {
        ...
        parentNode.add(new Node(this.currentLine++, expression.type, result.leftName, '', result.rightName));
    }
    ...
    whileStatementNode(data, parentNode) {
        ...
        let newNode    = new Node(this.currentLine++, data.type, 'while', this.conditionRecursive(conditions));
    ...
    ifStatementNode(data, parentNode) {
        ...
        let newNode = new Node(this.currentLine++, data.type, 'ElseIfStatement' === data.type ? 'else if' : 'if', test);
        parentNode.add(newNode);
        ...
        if (data.alternate) {
            if ('IfStatement' === data.alternate.type) {
                ...
                let newNode = new Node(this.currentLine++, 'Else', 'else');
                parentNode.add(newNode);
    ...
    returnStatementNode(data, parentNode) {
        ...
        parentNode.add(new Node(this.currentLine++, data.type, 'return', '', result));
    }
        ...
        for (let i = 0; i < this.nodes.length; i++) {
            ...
            for (let node of this.nodes[i].childreen) {
                if ('IfStatement' === node.type || 'ElseIfStatement' === node.type || 'WhileStatement' === node.type) {
                    if (this.evaluateIt(node.substitution)) {
                        node.color  = 'green';
                        thereIsTrue = !thereIsTrue ? true : false;
                    } else node.color = 'red';
                }
            }
    ...
    render() {
        ...
        for (let i = 0; i < this.nodes.length; i++) {
            string += 'VariableDeclaration' === this.nodes[i].type ? this.nodes[i].variableKind + ' ' + this.nodes[i].title + ' = ' + this.nodes[i].value + ';<br>' : '';
            string += 'FunctionDeclaration' === this.nodes[i].type ? this.nodes[i].title : '';
            string += this.nodes[i].childreen && this.nodes[i].childreen.length ? this.renderRecursive(this.nodes[i], 1) : '';
        }
    ...
    // Build & Render the HTML
    renderRecursive(parentNode, depth) {
        let string = ' {<br>';
        let tabs   = this.tabs(depth);
        for (let node of parentNode.childreen) {
            string += ('IfStatement' === node.type || 'ElseIfStatement' === node.type || 'WhileStatement' === node.type) ? tabs + '<span class="' + node.color + '">' + node.title + '(' + node.substitution + ')</span>' : '';
            string += 'Else' === node.type ? tabs + '<span class="' + node.color + '">' + node.title + '</span>' : '';
            string += ('AssignmentExpression' === node.type && typeof this.arguments[node.title] !== 'undefined') ? tabs + node.title + ' = ' + node.substitution + ';<br>' : '';
            string += 'ReturnStatement' === node.type ? tabs + node.title + ' ' + node.substitution + ';' : '';
            string += node.childreen && node.childreen.length ? this.renderRecursive(node, depth + 1) : '';
        }
        string += '<br>' + this.tabs(depth - 1) + '}<br>';
